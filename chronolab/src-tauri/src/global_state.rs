...
use std::{fs::File, io::{Write, BufReader}, path::Path, sync::Mutex};
...
pub enum AppStateField {
    SaveFilePath { value: Option<SafePathBuf>},
...
impl AppState {
    ...
    fn get_field(&self, field: AppStateField) -> AppStateField {
        match field {
            AppStateField::SaveFilePath{..} => AppStateField::SaveFilePath{value: self.save_file_path.clone() },
            AppStateField::CsvFilePath{..} => AppStateField::CsvFilePath{value: self.csv_file_path.clone() },
            AppStateField::LoadCsvSettings{..} => AppStateField::LoadCsvSettings{ value: self.load_csv_settings.clone() },
            AppStateField::VideoFilePath{..} => AppStateField::VideoFilePath{ value: self.video_file_path.clone() },
            AppStateField::VideoStartTime{..} => AppStateField::VideoStartTime{ value: self.video_start_time.clone() },
            AppStateField::IsMultiwindow{..} => AppStateField::IsMultiwindow{ value: self.is_multiwindow.clone() },
        }
    }
    ...
    pub fn load_from_file(path: &Path) -> Result<Self, String> {
        ...
        let app_state = serde_json::from_reader(reader).map_err(|err| format!("Failed to deserialize JSON: {}", err))?;
...
pub async fn set_app_state<'a>(
    ...
    let mut app_state= state
        .lock()
        .map_err(|e| format!("Error locking app state in set_app_state: {}", e.to_string()))?;
...
fn emit_app_state_update<T: Serialize + Clone>(app: AppHandle, field_name: String, event_payload: T) -> Result<(), String> {
...
pub async fn get_app_state<'a>(
    ...
    let app_state = state
        .lock()        
        .map_err(|e| format!("Error locking app state in get_app_state: {}", e.to_string()))?;
...
fn to_json<T: Serialize>(payload: T, field_name: String) -> Result<Value, String> {
    let value_json = serde_json::to_value(payload)
        .map_err(|err| format!("Function get_app_state in global_state.rs -- failed to serialize {}: {}", field_name, err))?;
    Ok(value_json)
}

/// Clear the current settings and create a new one.
#[tauri::command]
pub async fn clear_app_state<'a>(state: State<'a, Mutex<AppState>>) -> Result<(), String> {
        
    let mut app_state = state
        .lock()
        .map_err(|e| format!("Error locking app state in clear_app_state: {}", e.to_string()))?;
...
pub async fn save_app_state_to_file<'a>(state: State<'a, Mutex<AppState>>) -> Result<(), String> {
    ...
    let app_state = state
        .lock()
        .map_err(|e| format!("Error locking app state in save_app_state_to_file: {}", e.to_string()))?;
...
#[tauri::command]
pub async fn load_app_state_from_file<'a>(app: AppHandle, state: State<'a, Mutex<AppState>>, file: SafePathBuf) -> Result<(), String> {
    let mut app_state: std::sync::MutexGuard<'_, AppState> = state
        .lock()
    ...
    *app_state = AppState::load_from_file(file.as_ref())?; 
    ...
    for app_state_field in AppStateField::iter() {
        let field_name = app_state_field.to_string();
        match app_state_field {
            AppStateField::SaveFilePath { value} => emit_app_state_update(app.clone(), field_name, value)?,
            AppStateField::CsvFilePath { value} => emit_app_state_update(app.clone(), field_name, value)?,
            AppStateField::LoadCsvSettings{ value } => emit_app_state_update(app.clone(), field_name, value)?,
            AppStateField::VideoFilePath{ value } => emit_app_state_update(app.clone(), field_name, value)?,
            AppStateField::VideoStartTime{ value } => emit_app_state_update(app.clone(), field_name, value)?,
            AppStateField::IsMultiwindow{ value } => emit_app_state_update(app.clone(), field_name, value)?,
        };
    }
